...
$(function () {
	...
	$("#send_form").on('submit', function () {
		...
		var brand_website = $("#brand_website").val().trim();
		var biggest_brand = $("#biggest_brand").val().trim();
		var big_goal = $("#big_goal").val().trim();

		$.ajax({
			url: 'mail.php',
			type: 'POST',
			cache: false,
			data: { 'name': name, 'email': email, 'phone_number': phone_number, 'brand_website': brand_website, 'biggest_brand': biggest_brand, 'big_goal': big_goal},
			dataType: 'html',
			beforeSend: function () {
